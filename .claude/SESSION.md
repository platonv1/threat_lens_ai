...
Phase 6 is complete (backend + frontend). Other open items:
...
- `verdict` (`safe`/`low-risk`/`suspicious`/`dangerous`) is in the same position `scan_type` was in before Phase 6 — a free-text column with a small fixed set of real values, not yet formalized as an enum. Not tackled to keep scope tight; same treatment would apply if it's worth doing. Also noticed in passing: `RiskMeter.tsx`'s `VERDICT_STYLES` only maps `safe`/`suspicious`/`dangerous`, missing `low-risk` (falls back to a neutral gray rather than erroring) — cosmetic, not fixed.
- `docs/UI_UX.md`'s "Settings" page still has no backing feature (pre-existing gap, separate from History).
- Phase 7 (Reports) is the only remaining phase.