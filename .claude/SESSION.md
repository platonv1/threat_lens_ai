...
Five minor findings were deferred (not fixed) during this feature's task reviews; a few of the new tests lean on light coverage of edge cases rather than being exhaustive. None of these block this feature; each was a candidate for a future pass if reports become more heavily used.

**Final whole-branch review (this session)** caught two confirmed bugs the per-task reviews had missed, both contradicting the feature's own graceful-degradation design: a present-but-corrupt/undecodable upload file raised `PIL.UnidentifiedImageError` out of `generate_report` instead of falling back to "Image unavailable." (reportlab defers image decode to `doc.build()` by default, so the fix needed `lazy=0` to force eager decode inside the `try/except`); and an unbroken string longer than a table column (e.g. an unvalidated long hostname in a DNS finding) raised a `reportlab` `LayoutError`. Fixed in commit `37d05d1`: an `Image(...)` `try/except` fallback, and a `_MAX_FIELD_LENGTH = 500`-char truncation applied to `input_text`/`check`/`severity`/`finding` before rendering. Also fixed in that commit: the route's OpenAPI schema wrongly advertised `application/json` instead of `application/pdf` (`response_class=Response` was missing), and the image-embedding test was strengthened to assert `/Subtype /Image` presence/absence rather than just `%PDF-` magic bytes. 85/85 backend tests passing after the fix. Both new regression tests were confirmed to fail against the pre-fix code before the fix made them pass.

### Stale `backend` image after Phase 7 merge — fixed, and Report Export re-verified (this session)

Debugging `curl: (56) Recv failure: Connection reset by peer` against `GET /health` traced (via `docker compose logs backend`) to `ModuleNotFoundError: No module named 'reportlab'` crashing Uvicorn's app import on every worker restart — the socket accepted TCP connections but reset them since no app ever came up. Root cause: `reportlab` was already correctly listed in `backend/requirements.txt` (added during Phase 7), but the running `backend` container's image predated that change and was never rebuilt — the same class of stale-image issue hit twice before in this project's history (`httpx`/`dnspython`/`python-whois` gaps noted earlier in this file). Fixed with `docker compose build backend && docker compose up -d backend`; confirmed clean startup logs and `GET /health` → `200 {"status":"ok"}`, including after a subsequent `docker compose restart backend` (rules out a fluke of the freshly-built container still being warm).

With the backend healthy again, re-verified Report Export end-to-end via `curl`: `POST /scan/sms` (a real shortened-link phishing text) → `id=15`, real Ollama-generated `ai_summary`; `GET /scan/15/report` → `200`, `application/pdf`, correct `Content-Disposition`, 2545 bytes, confirmed a genuine well-formed PDF (`file` reports `PDF document, version 1.4, 1 pages`) and sent to the user directly for a visual check; `GET /scan/999999/report` → `404` as expected. Did not re-exercise the corrupt-image/oversized-text fallback paths from commit `37d05d1` this session — those already have dedicated regression tests and a prior real-container verification; out of scope for what was asked here.
...
No tests needed for the doc/wording-only changes; backend test suite re-run to confirm the DNS wording change didn't break anything. Committed (`c44de8c`) and pushed to `origin/main`.

### EC2 instance unreachable; "Updating the Application" section rewritten (this session)

Checked the deployed instance's health post-push at the IP recorded in the guide's NOTES section (`3.27.171.122`): `curl`, `ping`, and raw `nc` to ports 80/22 all timed out with no response at all — not an app-level 502/504 like the earlier incidents, but total unreachability at the network layer (instance likely stopped, Elastic IP reassociated, or security group changed). Not root-caused this session — no AWS console/CLI access available from this environment; flagged to the user to check the instance state directly. **Also confirmed there is no CI/CD**: a `git push` to `main` only updates GitHub, nothing auto-deploys to EC2.

Prompted by that gap, rewrote `docs/AWS_Deployment_Guide.md`'s stale "Updating the Application" section (previously a generic, pre-systemd "git pull, restart the application" outline with no real detail) into a full step-by-step: push → SSH in → `git pull` → install any new backend/frontend deps (`pip install -r requirements.txt` / `npm install`) → run new Alembic migrations if any → rebuild the frontend if it changed → restart the two `systemd` services → verify via `curl`, with a troubleshooting fallback to `systemctl status`/`journalctl` if verification fails. Explicitly calls out, with references to this file's own history, the two failure modes this project has already hit for real: a new dependency pulled but not installed (silent crash-loop, the `reportlab`/`ModuleNotFoundError` incident) and a new migration pulled but not applied (starts fine, breaks on first request, the `/history` `UndefinedTable` incident). Phase 13b §6 (which covered the same ground more thinly) now just points to this section instead of duplicating it. Added a short pointer in `docs/LOCAL_APP_GUIDE.md` ("Deploying to production") since that guide is local-only and a reader might otherwise not realize `git push` doesn't deploy. Documentation-only — no application code changed, and not yet exercised against a real deploy this session (the target instance is unreachable, see above).

### Mobile "site can't be reached" — HTTPS was never enabled on production (this session)

User reported `http://www.cyberscamshieldai.site/` unreachable from a phone. Diagnosed from the Mac with `dig`/`curl`/`nc`: DNS resolved correctly and port 80 returned a healthy `200`, but port 443 returned `Connection refused` — nginx had no listener on 443 at all. Root cause: `docs/AWS_Deployment_Guide.md` Phase 15 ("Enable HTTPS") was always just a 3-line stub ("Install Let's Encrypt using Certbot") that was never actually carried out against the live instance. Mobile browsers (Chrome/Safari) default to trying `https://` first and show a hard error instead of silently falling back to HTTP the way `curl` does, which is why this only surfaced as a mobile-specific complaint even though desktop `curl` against explicit `http://` worked the whole time.

Fixed by actually running Phase 15 against the EC2 instance (`13.54.243.90`, via the `cyber_scam_shield_key` SSH config entry):
1. `/etc/nginx/sites-available/default`'s catch-all `server_name _;` changed to the real hostnames (`cyberscamshieldai.site www.cyberscamshieldai.site`) so Certbot's `--nginx` plugin could match the right server block — backed up the original file first, validated with `nginx -t` before reloading.
2. Installed Certbot via snap (`sudo snap install --classic certbot`), then `sudo certbot --nginx -d cyberscamshieldai.site -d www.cyberscamshieldai.site` with the HTTP→HTTPS redirect option — Certbot added the `listen 443 ssl` block plus a second `server` block that 301-redirects port 80 to HTTPS, and reloaded nginx itself.
3. Verified end-to-end from the Mac: `https://www.cyberscamshieldai.site/` and the apex domain both return `200` over TLS 1.3 with a cert matching the hostname (Let's Encrypt, expires 2026-11-04); plain `http://` now 301-redirects to `https://`.
4. Confirmed auto-renewal: `sudo certbot renew --dry-run` succeeded, and the `snap.certbot.renew.timer` systemd timer is active (runs twice daily).

Ops-only change, no application code touched. `docs/AWS_Deployment_Guide.md` Phase 15 is still just the 3-line stub — worth fleshing out into real steps (mirroring what was actually done above) so this doesn't have to be re-derived next time; not done yet this session.

### SSL/WHOIS finding wording made plain-language (this session)

Continuation of the DNS wording pass from an earlier session — `dns_service.py` was already reworded to plain, scam-framed language, but `ssl_service.py` and `whois_service.py` still returned raw technical strings ("Valid HTTPS certificate.", "Domain registered 1928 days ago.") that don't mean much to a non-technical end user. Reworded both to match the DNS finding's tone and structure (reassuring language for safe results, "a common warning sign of a scam or fake site" framing for risky ones):

- `backend/app/services/ssl_service.py`: all four `Finding.message`s reworded (no-HTTPS, valid cert, invalid/untrusted cert, connection failure) — `severity`/`check` unchanged.
- `backend/app/services/whois_service.py`: added a `_humanize_age()` helper converting raw day counts into readable durations ("5 days ago" / "3 months ago" / "5 years ago" instead of e.g. "1928 days ago"), and reworded all four outcomes (very new/somewhat new/established/lookup-failed). The lookup-failed fallback message still contains the substring "unavailable" since `test_whois_service.py::test_lookup_failure_falls_back_to_info` asserts on that.
